...
//  Homework4KM
//
//  Created by Kristina Myftaraga on 3/5/20.
...
int main() {
    ...
    Division * homeAppliance = new Division("ABCD-EFGH-0001", "Home Appliances", "069   223","Home appliances are electrical / mechanical machines which accomplish some household functions, such as cooking, cleaning, or food preservation");
...
*/
    ...
    vector<pair<int,Item*> > allItems;//used the vecor of pair to sort in decreasiing order
    //a loop to calculate and display the total prices of all elements in the Items collection
    for (Item * i : items) {
            string UpperString = i -> getName();
            ...
            if(CurrOccurances>0)
              allItems.push_back(make_pair(CurrOccurances,i));

    }
    sort(allItems.begin(),allItems.end()); // sort them from number or occurences
    reverse(allItems.begin(),allItems.end()); // start from biggest
    if(allItems.size()==0)cout<<"No items contains the string: "<<finder1<<endl;
    else cout<<allItems.size()<< " items found...\n";
    for(auto curr: allItems){
      cout<< " > "<< curr.second->getName() <<" ("<< curr.first << ")\n";
      cout<< "  - "<<curr.second->getGUID()<<endl;
    }
